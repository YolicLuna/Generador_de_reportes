...
def generar_reporte(periodo, conexion, config):
    """
    Genera un reporte en formato Word basado en el periodo especificado.
    """
    ...
    # Ejecutar las consultas y agregar los resultados al documento
    for consulta in consultas:
        document.add_heading(consulta["titulo"], level=2)
        cursor.execute(consulta["query"])
        resultados = cursor.fetchall()
        columnas = [desc[0] for desc in cursor.description]

        # Crear una tabla en el documento
        table = document.add_table(rows=1, cols=len(columnas))
        hdr_cells = table.rows[0].cells
        for i, columna in enumerate(columnas):
            hdr_cells[i].text = columna
    ...
    nombre_archivo = f"Resultados_{periodo}_{datetime.now().strftime('%Y%m%d')}.docx"  # Para Word
    ruta_archivo = os.path.join(output_folder, nombre_archivo)
    document.save(ruta_archivo)  # Para Word
...
def main():
    """
    Función principal para generar reportes en Word.
    """
    try:
